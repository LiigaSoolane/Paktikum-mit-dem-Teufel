...
plt.plot(xl, unp.nominal_values(lnn(xl, *paramsl)), 'r--', label='Fit für Rh 104i')
plt.plot(xk, unp.nominal_values(lnn(xk, *paramsk)), 'y--', label='Fit für Rh 104')
plt.plot(tr, np.log(unp.nominal_values(unp.exp(lnn(tr, lambl, constl)) + unp.exp(lnn(tr, lambk, constk)))), 'g-', label='Kombination beider Fits')
plt.yscale('log')
plt.xlabel('t [s]')
plt.ylabel('ln(N)')
plt.legend()
