...
plt.xlabel(r"$\alpha_1$ [deg]")
plt.ylabel(r"$\alpha_2$ [deg]")
plt.legend(loc='best')
plt.tight_layout()
...
n = np.sin(alpha*math.pi/180.0)/np.sin(beta*math.pi/180.0)
nmid = np.mean(n)
nerr = sem(n)
...
s = d*np.sin((alpha-beta)*math.pi/180)/np.cos(beta*math.pi/180)
...
alpha = (alpha*math.pi/180)
alphag = (alphag*math.pi/180)
alphar = (alphar*math.pi/180)
...
print(tabulate (table, tablefmt="latex"))
for i in range(2):
    lambd[i]=d*np.sin(phi[i]*math.pi/180)
...
print(tabulate (table, tablefmt="latex"))
for i in range(2):
    lambd[i+2]=d*np.sin(phi[i][0]*math.pi/180)/(i+1)
    lambd[i+4]=d*np.sin(phi[i][1]*math.pi/180)/(i+1)
...
print(tabulate (table, tablefmt="latex"))
for i in range(4):
    lambd[i+6]=d*np.sin(phi[i][0]*math.pi/180)/(i+1)
    lambd[i+10]=d*np.sin(phi[i][1]*math.pi/180)/(i+1)
